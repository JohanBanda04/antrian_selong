...
where tgl_input BETWEEN '$tgl_dari_default' and '$tgl_sampai_default' group by kode_satker"));
    ...
    public function gantipassword(Request $request, $kode_user)
    {
        //return $kode_user;
        $getuser = DB::table('salaman')->where('kode_user', $kode_user)->first();
        return view('user.updateprofile', compact('getuser'));
    }

    public function updatepassword(Request $request, $kode_user)
    {
        ...
        $datauser = DB::table('salaman')->where('kode_user', $kode_user)->first();
        $old_password = Hash::make($request->password_lama);

        $confirm_old_password_plaintext = $request->password_lama;
        $old_password_encrypted = $datauser->password;

        if (Hash::check($confirm_old_password_plaintext, $old_password_encrypted)) {
            ...
            if ($password_new == $password_new_confirm) {
                try {
                    ...
                    $update = DB::table('salaman')->where('kode_user', $kode_user)->update($data_to_update);
                    if ($update) {
                        return redirect()->route('gantipassword', $kode_user)->with(['success' => 'Data Berhasil Diupdate']);
                    } else {
                        return redirect()->route('gantipassword', $kode_user)->with(['warning' => 'Data Gagal Diupdate']);
                    }
                } catch (Exception $e) {
                    return redirect()->route('gantipassword', $kode_user)->with(['warning' => 'Data Gagal Diupdate, error : ' . $e]);
        ...
        } else if (!Hash::check($confirm_old_password_plaintext, $old_password_encrypted)) {
            return redirect()->route('gantipassword', $kode_user)->with(['warning' => 'Data Gagal Diupdate, Password Lama Anda Salah']);
        }
        //dd($datauser);
