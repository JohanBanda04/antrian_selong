...
class RuanganController extends Controller
{
    ...
    public function ruangandata(Request $request)
    {
        $default_jenisruangan = "";
        $ruangans = DB::table('ruangan');

        if (!empty($request->jenis_ruangan_cari)) {
            $default_jenisruangan = $request->jenis_ruangan_cari;
            $ruangans->where('jenis_ruangan', $default_jenisruangan);
        }

        if (!empty($request->nama_ruangan_cari)) {
            $ruangans->where('name_ruangan', 'like', '%' . $request->nama_ruangan_cari . "%");
        }

        $ruangans->orderBy('created_at', 'desc');
        $dtRuangans = $ruangans->paginate(5);

        $results = DB::select(DB::raw("SELECT MAX(right(kode_ruangan,2)) as kode FROM ruangan"));

        $kode_new = $results[0]->kode + 1;
        $kode_ruangan = "KWR-NTB-0" . $kode_new;

        return view('ruangan.index', compact('dtRuangans', 'kode_ruangan', 'default_jenisruangan'));
    }
    ...
    public function ruanganstore(Request $request)
    {
        ...
        if ($request->hasFile('foto_ruangan')) {
            $foto_ruangan = $kode_ruangan . "." . $request->file('foto_ruangan')->getClientOriginalExtension();
        } else {
            $foto_ruangan = null;
        }

        $cek_ruangan_backup = DB::table('ruangan')
            ->where('name_ruangan', $nama_ruangan)
            ->orWhere('kode_ruangan', $kode_ruangan);

        $cek_ruangan= DB::table('ruangan')
            ->where('kode_ruangan', $kode_ruangan);
        if ($cek_ruangan->count() > 0) {
            //echo "data sudah ada";
            return redirect()->route('ruangandata')->with(['warning' => 'Gagal Menyimpan!! Nama Ruangan Sudah Ada / Kode Ruangan Telah Terdaftar']);
        } else {
            //echo "data belum ada";
            try {
                $data = [
                    "kode_ruangan" => $kode_ruangan,
                    "name_ruangan" => $nama_ruangan,
                ...
                if ($simpan) {
                    if ($request->hasFile('foto_ruangan')) {
                        $folderPath = "public/uploads/salamanruangan";
                        $request->file('foto_ruangan')->storeAs($folderPath, $foto_ruangan);
                    }
                    return redirect()->route('ruangandata')->with(['success' => 'Data Berhasil Disimpan']);
                } else {
                    return redirect()->route('ruangandata')->with(['warning' => 'Data Gagal Disimpan']);
                }
            } catch (Exception $e) {
                return redirect()->route('ruangandata')->with(['warning' => 'Data Gagal Disimpan Bor, error: ' . $e]);
    ...
    public function ruanganedit(Request $request)
    {
        $kode_ruangan = $request->kode_ruangan;
        //echo $request->kode_ruangan;
        //return $kode_ruangan;
    ...
    public function ruanganupdate(Request $request, $kode_ruangan)
    {
        //return $kode_ruangan;
        $kode_ruangan_edit = $request->kode_ruangan_edit;
        $nama_ruangan_edit = $request->nama_ruangan_edit;
        $jenis_ruangan_edit = $request->jenis_ruangan_edit;
        $old_foto_ruangan_edit = $request->old_foto_ruangan_edit;
        if ($request->hasFile('foto_ruangan_edit')) {
            $foto_edit = $kode_ruangan_edit . "." . $request->file('foto_ruangan_edit')->getClientOriginalExtension();
        } else {
            $foto_edit = $old_foto_ruangan_edit;
        }
        $get_data_old = DB::table('ruangan')->where('kode_ruangan', $kode_ruangan_edit)->first();
        $get_old_name_ruangan = $get_data_old->name_ruangan;
        $simpan = "n";
        //return $get_old_name_ruangan;
        if ($get_old_name_ruangan == $request->nama_ruangan_edit) {
            $nama_ruangan_edit = $request->nama_ruangan_edit;
            $simpan = "y";
        } else if ($get_old_name_ruangan != $request->nama_ruangan_edit) {
            $nama_ruangan_edit = $request->nama_ruangan_edit;
            $cek_ruangan = DB::table('ruangan')
                ->where('name_ruangan', $nama_ruangan_edit);
            if ($cek_ruangan->count() > 0) {
                $simpan = "n";
            } else if ($cek_ruangan->count() <= 0) {
                $simpan = "y";
            }
        }

        if ($simpan == "n") {
            return redirect()->route('ruangandata')->with(['warning' => "Nama Ruangan dengan nama " . strtoupper($request->nama_ruangan_edit) . " telah terdaftar sebelumnya"]);

        } else if ($simpan == "y") {
            try {
                $data = [
                    "name_ruangan" => $nama_ruangan_edit,
                    "jenis_ruangan" => $jenis_ruangan_edit,
                    "foto" => $foto_edit,
                    "updated_at" => date('Y-m-d H:i:s'),
                ];
                $update = DB::table('ruangan')->where('kode_ruangan', $kode_ruangan_edit)->update($data);
                if($update){
                    if ($request->hasFile('foto_ruangan_edit')) {
                        $folderPath = "public/uploads/salamanruangan";
                        $folderPathOld = "public/uploads/salamanruangan/" . $old_foto_ruangan_edit;

                        /*hapus dulu foto lama*/
                        Storage::delete($folderPathOld);

                        /*upload kemudian foto baru*/
                        $request->file('foto_ruangan_edit')->storeAs($folderPath, $foto_edit);
                    }
                    return redirect()->route('ruangandata')->with(['success' => 'Data Ruangan Berhasil di-Update']);
                } else {
                    return redirect()->route('ruangandata')->with(['warning' => 'Data Ruangan Gagal di-Update']);
                }
            } catch (Exception $e) {
                return redirect()->route('ruangandata')->with(['warning' => 'Data Ruangan Gagal di-Update, error : '.$e]);
            }
        }

    }

    public function ruangandelete($kode_ruangan){
        //return $kode_ruangan;
        $ruangan = DB::table('ruangan')->where('kode_ruangan', $kode_ruangan)->first();
        $foto_to_delete = $ruangan->foto;
        $folderPathOld = "public/uploads/salamanruangan/" . $foto_to_delete;
        $delete = DB::table('ruangan')->where('kode_ruangan', $kode_ruangan)->delete();
        if($delete){
            Storage::delete($folderPathOld);
            return redirect()->route('ruangandata')->with(['success'=>'Data Berhasil Dihapus!']);
        } else {
            return redirect()->route('ruangandata')->with(['warning'=>'Data Gagal Dihapus!']);
        }

    }

    public function tampilkandetailruangan(Request $request){
        $kode_ruangan =  $request->kode_ruangan;
        //return $kode_ruangan;
        $dataruangan = DB::table('ruangan')->where('kode_ruangan',$kode_ruangan)->first();
        return view('ruangan.detail',compact('dataruangan'));
    }
