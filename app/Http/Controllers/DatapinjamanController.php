...
//        echo count($url_gabung);
    ...
    public function cetakantrian_kunjungan_bk(Request $request)
    {
        $get_absenlog = DB::table('absenlog')->get();
        $hari_ini = date('Y-m-d');
        $last_nomorantrian = "";
        $last_absen = DB::select(DB::raw("SELECT MAX(right(no_antrian,3)) as no_antrian FROM absenlog where created_at='$hari_ini' and jenis_antrian='Kunjungan'"));
        foreach ($last_absen[0] as $dt){
          $last_nomorantrian = $dt;
        }
        $no_antrian_only = explode('-', $last_nomorantrian)[1];
        $create_new_noantrian = $no_antrian_only+1;
        $final_noantrian= "K-".$create_new_noantrian;
        $tanggal_hari_ini = date("d-m-Y");

        return view('cetakantrian.cetakantrian_kunjungan', compact('create_new_noantrian','final_noantrian','tanggal_hari_ini'));

    }
    public function cetakantrian_kunjungan(Request $request)
    {
        $get_absenlog = DB::table('absenlog')->get();
        $hari_ini = date('Y-m-d');
        $cek_today_existence = DB::select(DB::raw("SELECT * FROM absenlog where created_at='$hari_ini' and jenis_antrian='Kunjungan'"));

        if(sizeof($cek_today_existence)<=0){
            $last_nomorantrian = "";
            $last_absen = DB::select(DB::raw("SELECT MAX(right(no_antrian,3)) as no_antrian FROM absenlog where created_at='$hari_ini' and jenis_antrian='Kunjungan'"));
            //echo sizeof($last_absen); die;
            foreach ($last_absen[0] as $dt){
                $last_nomorantrian = $dt;
            }
            $no_antrian_only = 0;
        ...
        } else if(sizeof($cek_today_existence)>0){
            $last_nomorantrian = "";
            $last_absen = DB::select(DB::raw("SELECT MAX(right(no_antrian,3)) as no_antrian FROM absenlog where created_at='$hari_ini' and jenis_antrian='Kunjungan'"));
            //echo sizeof($last_absen); die;
            foreach ($last_absen[0] as $dt){
        ...
        $data_tosave = [
            'no_antrian'=>$final_noantrian,
            'jenis_antrian'=>'Kunjungan',
            'status'=>'Menunggu',
            'created_at'=>date('Y-m-d'),
            'updated_at'=>date('Y-m-d'),
        ];
        $simpan = DB::table('absenlog')->insert($data_tosave);
        return view('cetakantrian.cetakantrian_kunjungan', compact('create_new_noantrian','final_noantrian','tanggal_hari_ini'));

    }
    public function cetakantrian_informasi(Request $request)
    {
        $get_absenlog = DB::table('absenlog')->get();
        $hari_ini = date('Y-m-d');
        $cek_today_existence = DB::select(DB::raw("SELECT * FROM absenlog where created_at='$hari_ini' and jenis_antrian='Informasi'"));

        if(sizeof($cek_today_existence)<=0){
            $last_nomorantrian = "";
            $last_absen = DB::select(DB::raw("SELECT MAX(right(no_antrian,3)) as no_antrian FROM absenlog where created_at='$hari_ini' and jenis_antrian='Informasi'"));
            //echo sizeof($last_absen); die;
            foreach ($last_absen[0] as $dt){
                $last_nomorantrian = $dt;
            }
            $no_antrian_only = 0;
            $create_new_noantrian = $no_antrian_only+1;
            $final_noantrian= "I-".$create_new_noantrian;
        ...
        $data_tosave = [
            'no_antrian'=>$final_noantrian,
            'jenis_antrian'=>'Informasi',
            'status'=>'Menunggu',
            'created_at'=>date('Y-m-d'),
            'updated_at'=>date('Y-m-d'),
        ];
        $simpan = DB::table('absenlog')->insert($data_tosave);
        return view('cetakantrian.cetakantrian_informasi', compact('create_new_noantrian','final_noantrian','tanggal_hari_ini'));

    }

    public function kunjungan_selesai(Request $request){
        $id_kunjungan = $request->id_kunjungan;
        try{
            $update = DB::table('absenlog')->where('id', $id_kunjungan)->update([
                'status'=>'Selesai',
                'updated_at'=>date('Y-m-d'),
            ]);
            if ($update) {
                //return redirect()->route('landingpage_loketkunjungan')->with(['success' => 'Data Berhasil Diupdate']);
                return redirect()->route('landingpage_loketkunjungan');
            } else {
                //return redirect()->route('landingpage_loketkunjungan')->with(['warning' => 'Data Gagal Diupdate']);
                return redirect()->route('landingpage_loketkunjungan');
            }
        } catch (\Exception $e){
            //return redirect()->route('landingpage_loketkunjungan')->with(['warning' => 'Data Gagal Diupdate']);
    ...
    public function informasi_selesai(){

    }


    public function cetakantrian_informasi_bk(Request $request)
    {
        //$jenis_antrian =
        return "cetak antrian informasi";
        return view('cetakantrian.cetakantrian_informasi');
    ...
    public function storepinjamkendaraan(Request $request)
    {
        ...
        if (count($cek_datacount) <= 0) {
            ...
            $last_bookingcode = DB::select(DB::raw("SELECT MAX(right(kode_booking,4)) as kode_booking FROM pinjam_kendaraan where created_at="));
...
where mulai_tgl='$mulai_tgl' and sampai_tgl='$sampai_tgl' and kode_kendaraan='$kode_kendaraan' and kode_booking='$kode_booking'"));
