...
class DatasubbidangController extends Controller
{
    public function datasubbidang(Request $request)
    {
        $default_namesubbidang = "";
        $subbidangs = DB::table('subbidang');
        if (!empty($request->nama_subbidang_cari)) {
            $default_namesubbidang = $request->nama_subbidang_cari;
            $subbidangs->where('name_subbid', 'like', '%' . $request->nama_subbidang_cari . '%');
        }
        $subbidangs->orderBy("kode_subbid", "desc");
        $dtSubbidangs = $subbidangs->paginate(5);
        $results = DB::select(DB::raw("SELECT MAX(right(kode_subbid,2)) as kode FROM subbidang"));
        $kode_new = $results[0]->kode + 1;
        $kode_subbid = "KW-NTB-0" . $kode_new;
        return view('subbidangs.index', compact('default_namesubbidang', 'dtSubbidangs', 'kode_subbid'));
    ...
    {
        //dd($request);
        $kode_subbid = $request->kode_subbid;
        $nama_subbid = $request->nama_subbid;
        $cek_subbid = DB::table('subbidang')
            ->where('name_subbid', $nama_subbid)
            ->orWhere('kode_subbid', $kode_subbid);
        if ($cek_subbid->count() > 0) {
            //echo "sudah ada nama subbit tersebut";
            return redirect()->route('datasubbidang')->with(['warning' => 'Gagal Menyimpan!! Nama subbidang :' . $nama_subbid . ' Sudah Ada Terdaftar']);
        } else if ($cek_subbid->count() <= 0) {
            //echo "belum ada nama subbit tersebut";
            try {
                $data_tosave = [
                    "name_subbid" => $nama_subbid,
                    "kode_subbid" => $kode_subbid,
                    "created_at" => date('Y-m-d'),
                ];
                $simpan = DB::table('subbidang')->insert($data_tosave);
                if ($simpan) {

                    return redirect()->route('datasubbidang')->with(['success' => 'Data Berhasil Disimpan']);
                } else {
                    return redirect()->route('datasubbidang')->with(['warning' => 'Data Gagal Disimpan']);
                }
            } catch (Exception $e) {
                return redirect()->route('datasubbidang')->with(['warning' => 'Data Gagal Disimpan Bor, error: ' . $e]);
            }
        }
        //die;
    ...
    public function subbidangedit(Request $request)
    {
        $kode_subbidang = $request->kode_subbidang;
        $id_subbidang = $request->id_subbidang;
        $subbidang = DB::table('subbidang')->where('kode_subbid', $kode_subbidang)->where('id', $id_subbidang)->first();
        //echo "<pre>"; print_r($subbidang); die;
        return view('subbidangs.edit', compact('subbidang', 'kode_subbidang'));
    }

    public function userupdate(Request $request, $kode_user)
    {
        //echo $kode_user;
        $jenis_hakakses_edit = $request->jenis_hakakses_edit;
        $foto_user_edit = $request->foto_user_edit;
        $password_user_edit = $request->password_user_edit;
        $no_hp_edit = $request->no_hp_edit;
        $nama_user_edit = $request->nama_user_edit;
        $kode_user_edit = $request->kode_user_edit;
        $old_foto_user_edit = $request->old_foto_user_edit;

        if ($request->hasFile('foto_user_edit')) {
            $foto_edit = $kode_user_edit . "." . $request->file('foto_user_edit')->getClientOriginalExtension();
        } else {
            $foto_edit = $old_foto_user_edit;
        }


        $get_data_old = DB::table('salaman')->where('kode_user', $kode_user_edit)->first();
        $get_old_username_user = $get_data_old->username;
        $get_old_password_user = $get_data_old->password;
        $get_old_name_user = $get_data_old->name;
        $get_old_email_user = $get_data_old->email;
        $simpan = "n";

        if (!empty($password_user_edit)) {
            //echo "ada isian password, gunakan password baru";
            $password_tosave = Hash::make($request->password_user_edit);
        } else if (empty($password_user_edit)) {
            //echo "gunakan password lama";
            $password_tosave = $get_old_password_user;
            //echo $password_tosave;
        }
        //die;
        if ($get_old_username_user == $request->nama_user_edit) {
            $username_user_edit = $request->nama_user_edit;
            $simpan = 'y';
        } else if ($get_old_username_user != $request->nama_user_edit) {
            $username_user_edit = $request->nama_user_edit;
            $cek_user = DB::table('salaman')
                ->where('username', $username_user_edit);
            if ($cek_user->count() > 0) {
                $simpan = "n";
            } else if ($cek_user->count() <= 0) {
                $simpan = "y";
            }
        }

        if ($simpan == "n") {
            return redirect()->route('datauser')->with(['warning' => 'Username telah terdaftar']);
        } else if ($simpan == "y") {
            //echo $get_old_email_user; die;
            try {
                $update_tosave = [
                    "name" => $get_old_name_user,
                    "username" => $nama_user_edit,

                    "no_hp" => $no_hp_edit,
                    "password" => $password_tosave,
                    "foto" => $foto_edit,
                    "roles" => $jenis_hakakses_edit,
                    "updated_at" => date('Y-m-d H:i:s'),
                ];
                $update_datauser = DB::table('salaman')->where('kode_user', $kode_user_edit)->update($update_tosave);
                if ($update_datauser) {
                    if ($request->hasFile('foto_user_edit')) {
                        $folderPath = "public/uploads/salamanuser";
                        $folderPathOld = "public/uploads/salamanuser/" . $old_foto_user_edit;

                        /*hapus dulu foto lama*/
                        Storage::delete($folderPathOld);

                        /*upload kemudian foto baru*/
                        $request->file('foto_user_edit')->storeAs($folderPath, $foto_edit);
                    }
                    return redirect()->route('datauser')->with(['success' => 'Data User Berhasil di-Update']);
                } else {
                    return redirect()->route('datauser')->with(['warning' => 'Data User Gagal di-Update']);
                }
            } catch (Exception $e) {
                return redirect()->route('datauser')->with(['warning' => 'Data User Gagal di-Update, error : ' . $e]);
    ...
    public function subbidangupdate(Request $request, $kode_subbidang)
    {
        //dd($request);
        $id_subbid_edit = $request->id_subbid_edit;
        $kode_subbid_edit = $request->kode_subbid_edit;
        $nama_subbid_edit = $request->nama_subbid_edit;
        //echo $nama_subbid_edit; die;
        $get_data_old = DB::table('subbidang')
            ->where('kode_subbid', $kode_subbidang)
            ->where('id', $id_subbid_edit)->first();
        //dd($get_data_old);
        $get_old_nameof_subbid = $get_data_old->name_subbid;
        if ($get_old_nameof_subbid == $request->nama_subbid_edit) {
            $nameof_subbid_edit = $request->nama_subbid_edit;
            $simpan = 'y';
        } else if ($get_old_nameof_subbid != $request->nama_subbid_edit) {
            $nameof_subbid_edit = $request->nama_subbid_edit;
            $cek_subbid = DB::table('subbidang')
                ->where('name_subbid', $nameof_subbid_edit);
            if ($cek_subbid->count() > 0) {
                $simpan = "n";
            } else if ($cek_subbid->count() <= 0) {
                $simpan = "y";
        ...
        if ($simpan == "n") {
            return redirect()->route('datasubbidang')->with(['warning' => 'Nama Subbid yang anda Masukkan telah terdaftar']);
        } else if ($simpan == "y") {
            try {
                $data_tosave = [
                    "name_subbid" => $request->nama_subbid_edit,
                    "updated_at" => date('Y-m-d'),
                ];
                $update_datasubbid = DB::table('subbidang')
                    ->where('kode_subbid', $kode_subbid_edit)
                    ->where('id', $id_subbid_edit)->update($data_tosave);
                if ($update_datasubbid) {
                    return redirect()->route('datasubbidang')->with(['success' => 'Data Subbidang Berhasil di-Update']);
                } else {
                    return redirect()->route('datasubbidang')->with(['warning' => 'Data Subbidang Gagal di-Update']);

                }
            } catch (Exception $e) {
                return redirect()->route('datasubbidang')->with(['warning' => 'Data Subbidang Gagal di-Update, error :'.$e]);
            }
        }
    }

    public function subbidangdelete($kode_subbidang){
        $subbidang = DB::table('subbidang')->where('kode_subbid',$kode_subbidang)->first();
        $delete = DB::table('subbidang')->where('kode_subbid',$kode_subbidang)->delete();
        if($delete){
            return redirect()->route('datasubbidang')->with(['success'=>'Data Subbidang Berhasil Dihapus']);
        } else {
            return redirect()->route('datasubbidang')->with(['warning'=>'Data Subbidang Gagal Dihapus']);
        }

    }

    public function userdelete($kode_user)
    {
        $user = DB::table('salaman')->where('kode_user', $kode_user)->first();
        //echo "<pre>"; print_r($user); die;
        $foto_to_delete = $user->foto;
        $folderPathOld = "public/uploads/salamanuser/" . $foto_to_delete;
        $delete = DB::table('salaman')->where('kode_user', $kode_user)->delete();
        if ($delete) {
            Storage::delete($folderPathOld);
            return redirect()->route('datauser')->with(['success' => 'Data User Berhasil Dihapus']);
        } else {
            return redirect()->route('datauser')->with(['warning' => 'Data User Gagal Dihapus']);
        }
    }
