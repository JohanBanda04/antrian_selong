...
class PinjamRuanganController extends Controller
{
    ...
    {
        $default_kodebooking = "";
        $pinjamRuangans = DB::table('pinjam_ruangan');
        if (!empty($request->kode_booking_cariruangan)) {
            $default_kodebooking = $request->kode_booking_cariruangan;
            $pinjamRuangans->where('kode_booking', $request->kode_booking_cariruangan);
        }
        $pinjamRuangans->orderBy('created_at', 'desc');
        $dtpinjamRuangans = $pinjamRuangans->paginate(5);
        return view('pinjamruangan.index', compact('dtpinjamRuangans'));

    }

    public function pinjamruanganedit(Request $request)
    {
        $kode_booking_ruangan = $request->kode_booking_ruangan;
        //return $kode_booking_ruangan;
        $pinjamruangan = DB::table('pinjam_ruangan')->where('kode_booking', $kode_booking_ruangan)->first();
        return view('pinjamruangan.edit', compact('pinjamruangan'));
    }

    public function pinjamruanganupdate(Request $request, $kode_booking)
    {
        //echo "kode booking brey: ".$kode_booking; die;
        $keep_data = [];
        $keep_data2 = [];
        $keep_datakode_booking = [];
        $makesure_booked = [];
        $filter_booked_date = [];
        $filter_booked_date2 = [];
        $filter_available_date2 = [];
        $filter_available_date = [];
        $simpan_indikator = "n";
        $tanggal_gagal = "";
        $tanggal_gagal2 = "";

        $pilih_persetujuan = $request->pilih_persetujuan_ruangan;
        $kode_booking_pinjam = $request->kode_booking_pinjam;
        $id_kodebooking_pinjam = $request->id_kodebooking_pinjam;
        $mulai_tgl_pinjam = $request->mulai_tgl_pinjam;
        $mulai_tglpinjam_sql = $request->mulai_tglpinjam_sql;

        $sampai_tgl_pinjam = $request->sampai_tgl_pinjam;
        $sampai_tglpinjam_sql = $request->sampai_tglpinjam_sql;
        $kode_ruangan_dipinjam = $request->kode_ruangan_dipinjam;
        $get_dtruangan = DB::table('ruangan')->where('kode_ruangan', $kode_ruangan_dipinjam)->first();
        $get_nameruangan = $get_dtruangan->name_ruangan;
        $get_jenisruangan = $get_dtruangan->jenis_ruangan;

        if ($pilih_persetujuan == "Ditolak") {
            $cek_pinjamkendaraan = DB::select(DB::raw("select * from pinjam_ruangan 
where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_ruangan='$kode_ruangan_dipinjam' 
and kode_booking='$kode_booking_pinjam' and id='$id_kodebooking_pinjam'"));

            $jarakWaktu = abs(strtotime($mulai_tglpinjam_sql) - strtotime($sampai_tglpinjam_sql));
            $numberDays = $jarakWaktu / 86400;
            $numberDays = intval($numberDays) + 1;

            for ($i = 0; $i < $numberDays; $i++) {
                if ($i > 1) {
                    $tgl_torent = date('Y-m-d', strtotime($mulai_tglpinjam_sql . '+' . $i . ' days'));
                } else if ($i <= 1) {
                    $tgl_torent = date('Y-m-d', strtotime($mulai_tglpinjam_sql . '+' . $i . ' day'));
                }

                $delete = DB::table('pivot_pinjamruangan')
                    ->where('kode_ruangan', $kode_ruangan_dipinjam)
                    ->where('kode_booking', $kode_booking_pinjam)
                    ->where('tgl_pinjam', $tgl_torent)->delete();
            }
            try {
                $data_tosave = [
                    "status" => $pilih_persetujuan,
                    "updated_at" => date('Y-m-d'),
                ];

                $update = DB::table('pinjam_ruangan')
                    ->where('kode_booking', $kode_booking_pinjam)
                    ->where('id', $id_kodebooking_pinjam)->update($data_tosave);

                if ($update) {
                    return redirect()->route('pinjamruangan')->with(['success' => 'Data Berhasil Disimpan']);
                } else {
                    return redirect()->route('pinjamruangan')->with(['warning' => 'Data Gagal Disimpan']);
                }
            } catch (Exception $e) {
                return redirect()->route('pinjamruangan')->with(['warning' => 'Data Gagal Disimpan, error : ' . $e]);
            }
        } else if ($pilih_persetujuan == "Disetujui") {
            $cek_pinjamruangan = DB::select(DB::raw("select * from pinjam_ruangan 
where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_booking='$kode_booking_pinjam'"));
            $get_oldstatus = $cek_pinjamruangan[0]->status;
            $get_dataruangan = DB::table('ruangan')->where('kode_ruangan', $kode_ruangan_dipinjam)->first();
            $get_jenisruangan = $get_dataruangan->jenis_ruangan;
            $get_nameruangan = $get_dataruangan->name_ruangan;
            $jarakWaktu = abs(strtotime($mulai_tglpinjam_sql) - strtotime($sampai_tglpinjam_sql));
            $numberDays = $jarakWaktu / 86400;
            $numberDays = intval($numberDays) + 1;
            //echo $numberDays; die;
...
where tgl_pinjam='$tgl_torent' and kode_ruangan='$kode_ruangan_dipinjam' and kode_booking='$kode_booking_pinjam'"));
                if (count($cek_pivot_pinjamruangan) > 0) {
                    array_push($keep_data, (object)[
                        "status" => "Booked",
                        "tanggal" => $tgl_torent,
                        "kode_ruangan" => $kode_ruangan_dipinjam,
                        "kode_booking" => $kode_booking_pinjam,
                    ]);
                    ...
                    array_push($keep_data, (object)[
                        ...
                        "tanggal" => $tgl_torent,
                        "kode_ruangan" => $kode_ruangan_dipinjam,
                        "kode_booking" => "no bookingcode yet",
                    ]);
                }
            }
...
//            die;
            $filterBy = "Booked";
            $new_filtered = array_filter($keep_data, function ($var) use ($filterBy) {
                return ([
                    $var->status == $filterBy,
                    //$var->kode_booking == $kode_booking_pinjam,

                ]);
            });
            $makesure_by_thisbookingcode = array_filter($keep_data, function ($var) use ($kode_booking_pinjam) {
                return ($var->kode_booking == $kode_booking_pinjam);
            });

            //echo "<pre>"; print_r($makesure_by_thisbookingcode); die;
            //echo $get_oldstatus; die;
            if (($get_oldstatus == "Pengajuan Pinjaman") && (count($makesure_by_thisbookingcode) > 0)) {
                /*dia memang sudah punya bookingan di tbl pivot ruangan*/
                try {
                    $data_tosaved = [
                        "status" => $pilih_persetujuan,
                        "updated_at" => date('Y-m-d'),
                    ];
                    $update_pinjamruangan = DB::table('pinjam_ruangan')
                        ->where('kode_booking', $kode_booking_pinjam)
                        ->where('id', $id_kodebooking_pinjam)->update($data_tosaved);
                    if ($update_pinjamruangan) {
                        return redirect()->route('pinjamruangan')->with(['success' => "Berhasil Mengupdate Data"]);
                    } else {
                        return redirect()->route('pinjamruangan')->with(['warning' => "Gagal Mengupdate Data"]);
                    }
                } catch (Exception $e) {
                    return redirect()->route('pinjamruangan')->with(['warning' => "Berhasil Mengupdate Data, error :" . $e]);
                }

            } else if (($get_oldstatus == "Ditolak") && (count($makesure_by_thisbookingcode) <= 0)) {
                /*dia memang belum punya bookingan di tbl pivot kendaraan, tp pastikan apakah org lain jg tdk punya*/
                for ($i = 0; $i < $numberDays; $i++) {
                    if ($i > 1) {
                        $tgl_torent = date('Y-m-d', strtotime($mulai_tglpinjam_sql . '+' . $i . ' days'));
                    } else if ($i <= 1) {
                    ...
                    $cek_pivot_pinjamruangan2 = DB::select(DB::raw("select * from pivot_pinjamruangan 
where tgl_pinjam='$tgl_torent' and kode_ruangan='$kode_ruangan_dipinjam'"));
                    if (count($cek_pivot_pinjamruangan2) > 0) {
                        array_push($keep_data2, (object)[
                            "status" => "Booked",
                            "tanggal" => $tgl_torent,
                            "kode_ruangan" => $kode_ruangan_dipinjam,
                        ]);
                    } else if (count($cek_pivot_pinjamruangan2) <= 0) {
                        array_push($keep_data2, (object)[
                            "status" => "Available",
                            "tanggal" => $tgl_torent,
                            "kode_ruangan" => $kode_ruangan_dipinjam,
                ...
                //echo "<pre>"; print_r($keep_data2);die;
                $filterBy2 = "Booked";
                $new_filtered2 = array_filter($keep_data2, function ($var) use ($filterBy2) {
                    return ($var->status == $filterBy2);
                });

                /*foreach utk memilah yg booked dan available utk peringatan warning*/
                foreach ($keep_data2 as $index2 => $kd2) {
                    if ($kd2->status == "Booked") {
                        $get_dtruangan2 = DB::table("ruangan")
                            ->where("kode_ruangan", $kd2->kode_ruangan)->first();
                        array_push($filter_booked_date2, (object)[
                            "status" => "fix_booked",
                            "tgl_booked" => $kd2->tanggal,
                            "name_ruangan" => $get_dtruangan2->name_ruangan,
                            "jenis_ruangan" => $get_dtruangan2->jenis_ruangan,
                            "kode_ruangan" => $kd2->kode_ruangan,
                        ]);
                    } else if ($kd2->status == "Available") {
                        $get_dtruangan2 = DB::table("ruangan")
                            ->where("kode_ruangan", $kd2->kode_ruangan)->first();
                        array_push($filter_available_date2, (object)[
                ...
                if (count($new_filtered2) > 0) {
                    //echo "<br>ditolak to setuju gagal";
                    foreach ($filter_booked_date2 as $ind2 => $dta2) {
                        $tgl_booked = $dta2->tgl_booked;
                        $jenis_ruangan = $dta2->jenis_ruangan;
                        $tanggal_gagal2 .= $tgl_booked . ", ";
                        //echo $jenis_kendaraan."<br>";
                    }
                    $warning = "Gagal Menyetujui!!! Ruangan " . $get_nameruangan . " telah dibooking user lain pada " . $tanggal_gagal2 . '. Mohon memilih range tanggal peminjaman yang lain';
                    return redirect()->route('pinjamruangan')->with(['warning' => $warning]);
                } else if (count($new_filtered2) <= 0) {
                    //echo "<br>ditolak to setuju berhasil";
                    try {
                        $data_tosave = [
                            "status" => $pilih_persetujuan,
                            "updated_at" => date('Y-m-d'),
                        ...
                        $update_pinjamruangan = DB::table('pinjam_ruangan')
                            ...
                            ->where('id', $id_kodebooking_pinjam)->update($data_tosave);
                        if ($update_pinjamruangan) {
                            foreach ($filter_available_date2 as $idx2 => $dtx2) {
                                $data_pivotruangan = [
                                    "kode_ruangan" => $dtx2->kode_ruangan,
                                    "tgl_pinjam" => $dtx2->tgl_booked,
                                    "kode_booking" => $kode_booking_pinjam,
                                ];
                        ...
                        return redirect()->route('pinjamruangan')->with(['warning' => 'Data Gagal Disimpan, dgn error :' . $e]);

                    }
                }
            } else if (($get_oldstatus == "Disetujui") && (count($makesure_by_thisbookingcode) > 0)) {
                //echo "nothing to do yet"; die;
                return redirect()->route('pinjamruangan')->with(['success' => 'No change has been made']);
            }
        } else if ($pilih_persetujuan == "Pengajuan Pinjaman") {
            //echo "no action yet"; die;
            /*Pengajuan Pinjaman berarti dia memang sudah membooking di tbl pivot pinjamkendaraan*/
            $cek_pinjamruangan = DB::select(DB::raw("select * from pinjam_ruangan 
where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_booking='$kode_booking_pinjam'"));
            //echo "<pre>"; print_r($cek_pinjamruangan);die;
            $get_oldstatus = $cek_pinjamruangan[0]->status;
            $get_dataruangan = DB::table('ruangan')->where('kode_ruangan', $kode_ruangan_dipinjam)->first();
            $get_jenisruangan = $get_dataruangan->jenis_ruangan;
            $get_nameruangan = $get_dataruangan->name_ruangan;
            ...
            for ($i = 0; $i < $numberDays; $i++) {
                ...
                $cek_pivot_pinjamruangan = DB::select(DB::raw("select * from pivot_pinjamruangan 
where tgl_pinjam='$tgl_torent' and kode_ruangan='$kode_ruangan_dipinjam' and kode_booking='$kode_booking_pinjam'"));
                //echo "<pre>"; print_r($cek_pivot_pinjamruangan)."<br>";
            ...
            if (($get_oldstatus == "Disetujui") && (count($makesure_by_thisbookingcode) > 0)) {
                /*dia memang sudah punya bookingan di tbl pivot ruangan*/
                try {
                    $data_tosaved = [
                        "status" => $pilih_persetujuan,
                    ...
                    return redirect()->route('pinjamruangan')->with(['warning' => "Gagal Mengupdate Data, error :" . $e]);
                }
            } else if (($get_oldstatus == "Ditolak") && (count($makesure_by_thisbookingcode) <= 0)) {
                /*dia memang belum punya bookingan di tbl pivot kendaraan, tp pastikan apakah org lain jg tdk punya*/
                //echo "masuk sini gk lu"; die;
                for ($i = 0; $i < $numberDays; $i++) {
                    if ($i > 1) {
                        $tgl_torent = date('Y-m-d', strtotime($mulai_tglpinjam_sql . '+' . $i . ' days'));
                    } else if ($i <= 1) {
...
where tgl_pinjam='$tgl_torent' and kode_ruangan='$kode_ruangan_dipinjam'"));
                ...
                foreach ($keep_data2 as $index2 => $kd2) {
                    if ($kd2->status == "Booked") {
                        $get_dtruangans = DB::table("ruangan")
                            ->where("kode_ruangan", $kd2->kode_ruangan)->first();
                        array_push($filter_booked_date2, (object)[
                            "status" => "fix_booked",
                            "tgl_booked" => $kd2->tanggal,
                            "jenis_ruangan" => $get_dtruangans->jenis_ruangan,
                            "name_ruangan" => $get_dtruangans->name_ruangan,
                            "kode_ruangan" => $kd2->kode_ruangan,
                        ]);
                    } else if ($kd2->status == "Available") {
                        $get_dtruangans = DB::table("ruangan")
                ...
                if (count($new_filtered2) > 0) {
                    ...
                    foreach ($filter_booked_date2 as $ind2 => $dta2) {
                        $tgl_booked = $dta2->tgl_booked;
                        $jenis_ruangan = $dta2->jenis_ruangan;
                        $name_ruangan = $dta2->name_ruangan;
                        $tanggal_gagal2 .= $tgl_booked . ", ";
                    }
                    $warning = "Gagal Pengajuan Pinjaman!!! Ruangan " . $get_nameruangan . " dengan jenis " . $get_jenisruangan . " telah dibooking user lain pada " . $tanggal_gagal2 . '. Mohon memilih range tanggal peminjaman yang lain';
                    return redirect()->route('pinjamruangan')->with(['warning' => $warning]);
                } else if (count($new_filtered2) <= 0) {
                    try {
                        $data_tosave = [
                ...
                //die;
            } else if(($get_oldstatus == "Pengajuan Pinjaman") && (count($makesure_by_thisbookingcode) > 0)){
                return redirect()->route('pinjamruangan')->with(['success'=>'No change has been made for Pengajuan Pinjaman']);

            } else if (($get_oldstatus == "Ditolak") && (count($makesure_by_thisbookingcode) > 0)) {
                //echo "no action yet bro, harusnya ditolak";
                echo "no action yet bro, kondisi tdk memungkinkan jika status sebelumnya ditolak, maka pada pivot pinjaman, this booking tdk mgkn memiliki data pd tbl pivot pinjam kendaraan";
                die;
            }
            //echo "<pre>"; print_r($makesure_by_thisbookingcode); die;
    ...
    public function pinjamruanganupdate_bk(Request $request, $kode_booking)
    {
        $keep_data = [];
        $filter_booked_date = [];
        $filter_available_date = [];
        ...
        $kode_booking_pinjam = $request->kode_booking_pinjam;
        $id_kodebooking_pinjam = $request->id_kodebooking_pinjam;
        $perihal_pinjam = $request->perihal_pinjam_ruangan;
        $mulai_tglpinjam_sql = $request->mulai_tglpinjam_sql;
        $mulai_tgl_pinjam = $request->mulai_tgl_pinjam;
        $sampai_tglpinjam_sql = $request->sampai_tglpinjam_sql;
        $sampai_tgl_pinjam = $request->sampai_tgl_pinjam;
        ...
        $name_dan_jenis = $request->name_dan_jenis;
        $pilih_persetujuan_ruangan = $request->pilih_persetujuan_ruangan;
        //dd($request);

        if ($pilih_persetujuan_ruangan == "Ditolak") {
            //echo "Ditolak";
            //echo $mulai_tglpinjam_sql."<br>";
            //echo $sampai_tglpinjam_sql."<br>";
            $cek_pinjamruangan = DB::select(DB::raw("select * from pinjam_ruangan 
where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_ruangan='$kode_ruangan_dipinjam'"));
            $jarakWaktu = abs(strtotime($mulai_tglpinjam_sql) - strtotime($sampai_tglpinjam_sql));
            $numberDays = $jarakWaktu / 86400;
            ...
            try {
                $data_tosave = [
                    "status" => $pilih_persetujuan_ruangan,
                    "updated_at" => date('Y-m-d')
                ];
                $update_ruangan = DB::table('pinjam_ruangan')
                    ->where('kode_booking', $kode_booking_pinjam)
                    ->where('id', $id_kodebooking_pinjam)->update($data_tosave);
                if ($update_ruangan) {
                    return redirect()->route('pinjamruangan')->with(['success' => 'Data Berhasil Disimpan']);
        ...
        } else if ($pilih_persetujuan_ruangan == "Disetujui" || $pilih_persetujuan_ruangan == "Pengajuan Pinjaman") {
            //echo "Disetujui";
            /*gk dipake $cek_pinjamruangan*/
            $cek_pinjamruangan = DB::select(DB::raw("select * from pinjam_ruangan 
where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_ruangan='$kode_ruangan_dipinjam'"));
            $jarakWaktu = abs(strtotime($mulai_tglpinjam_sql) - strtotime($sampai_tglpinjam_sql));
            $numberDays = $jarakWaktu / 86400;
...
where tgl_pinjam='$tgl_torent' and kode_ruangan='$kode_ruangan_dipinjam'"));
            ...
            if (count($new_filtered) > 0) {
                $simpan_indikator = "n";
            } else if (count($new_filtered) <= 0) {
                $simpan_indikator = "y";
            }

            /*utk  dapetin tanggal-tanggal yang terbooking dan available guna menjadi patokan Warning*/
            foreach ($keep_data as $index => $kd) {
                if ($kd->status == "Booked") {
                    $get_jenis = DB::table("ruangan")
                        ->where("kode_ruangan", $kd->kode_ruangan)->first();
                    array_push($filter_booked_date, (object)[
                        "status" => "fix_booked",
                        "tgl_booked" => $kd->tanggal,
                        "jenis_ruangan" => $get_jenis->jenis_ruangan,
                        "kode_ruangan" => $kd->kode_ruangan,
                    ]);
            ...
            if ($simpan_indikator == "n") {
                foreach ($filter_booked_date as $ind => $dta) {
                    ...
                    $jenis_ruangan = $dta->jenis_ruangan;
                    $tanggal_gagal .= $tgl_booked . ", ";
                }
                $cek_pinjamruangan_data = DB::table('pinjam_ruangan')
                    ->where('mulai_tgl', $mulai_tglpinjam_sql)
                    ->where('sampai_tgl', $sampai_tglpinjam_sql)->get();
                $terbooking = DB::select(DB::raw("SELECT * from pinjam_ruangan where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_ruangan='$kode_ruangan_dipinjam' and status='Pengajuan Pinjaman'"));
                $kode_booking_otheruser = $terbooking[0]->kode_booking;

                $warning = "Gagal Menyetujui!!! Ruangan " . $name_ruangan . " telah dibooking user lain pada " . $tanggal_gagal . " dengan kode booking: " . $kode_booking_otheruser . '. Mohon memilih range tanggal peminjaman yang lain';
                return redirect()->route('pinjamruangan')->with(['warning' => $warning]);
            } else if ($simpan_indikator == "y") {
                /*LANJUTKAN DISINI BROJO*/
                try {
                    $data_tosave = [
                        "status" => $pilih_persetujuan_ruangan,
                        "updated_at" => date('Y-m-d'),
        ...
        } /*else if ($pilih_persetujuan_ruangan == "Pengajuan Pinjaman") {
            try {
                $data_tosave = [
                    "status" => $pilih_persetujuan_ruangan,
                    "updated_at" => date('Y-m-d'),
    ...
    public function pinjamruangandelete($kode_booking)
    {
        //echo $kode_booking;
        //die;
        //return $kode_booking;
        /*LANJUTKAN DISINI BROJO*/
        $pinjamruangan = DB::table('pinjam_ruangan')->where('kode_booking', $kode_booking)->first();
        $mulai_tglpinjam_sql = $pinjamruangan->mulai_tgl;
        $sampai_tglpinjam_sql = $pinjamruangan->sampai_tgl;
        $kode_ruangan_dipinjam = $pinjamruangan->kode_ruangan;
        ...
        $daduk_to_delete = $pinjamruangan->datadukung;
        $folderPathOld = "public/uploads/datadukungruangan/" . json_decode($daduk_to_delete);
        $delete = DB::table('pinjam_ruangan')->where('kode_booking', $kode_booking)->delete();
        if ($delete) {
            for ($i = 0; $i < $numberDays; $i++) {
                if ($i > 1) {
                    $tgl_torent = date('Y-m-d', strtotime($mulai_tglpinjam_sql . '+' . $i . ' days'));
            ...
            return redirect()->route('pinjamruangan')->with(['success' => 'Data Berhasil Dihapus!']);
        } else {
            return redirect()->route('pinjamruangan')->with(['warning' => 'Data Gagal Dihapus!']);
        }
        //die;
        //return $kode_booking;
    }

    public function tampilkandetailpinjamruangan(Request $request)
    {
        $kode_booking = $request->kode_booking;
        $id_booking = $request->id_booking;
        //return $kode_booking;
        $datapinjamruangan = DB::table('pinjam_ruangan')
            ->where('kode_booking', $kode_booking)
            ->where('id', $id_booking)->first();

        //echo "<pre>"; print_r($datapinjamruangan); die;
        //dd($datapinjamkendaraan);
        return view('pinjamruangan.detail', compact('datapinjamruangan'));
    }
    ...
    public function tampilkandetailkendaraan(Request $request)
    {
        $kode_kendaraan = $request->kode_kendaraan;
        //return $kode_kendaraan;
        $datakendaraan = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan)->first();
        return view('kendaraan.detail', compact('datakendaraan'));
    }
