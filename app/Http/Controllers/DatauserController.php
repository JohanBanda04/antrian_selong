...
class DatauserController extends Controller
{
    ...
    {
        $default_jenisuser = "";
        $users = DB::table('salaman');
        if (!empty($request->jenis_user_cari)) {
            $default_jenisuser = $request->jenis_user_cari;
            $users->where('roles', $default_jenisuser);
        }

        if (!empty($request->nama_user_cari)) {
            $users->where('name', 'like', '%' . $request->nama_user_cari . '%')
                ->orWhere('username', 'like', '%' . $request->nama_user_cari . '%');
        }
        $users->orderBy("created_at", "desc");
        $dtUsers = $users->paginate(5);

        $results = DB::select(DB::raw("SELECT MAX(right(kode_user,2)) as kode FROM salaman"));
        $kode_new = $results[0]->kode + 1;
        $kode_user = "USR-NTB-0" . $kode_new;
        //dd($kode_user);
        return view('users.index', compact('default_jenisuser', 'dtUsers', 'kode_user'));
    ...
    public function userstore(Request $request)
    {
        ...
        $jenis_hakakses = $request->jenis_hakakses;
        //dd($request);
        if ($request->hasFile('foto_user')) {
            $foto_user = $kode_user . "." . $request->file('foto_user')->getClientOriginalExtension();
        } else {
            $foto_user = null;
        }
        ...
        $cek_user = DB::table('salaman')
            ...
            ->orWhere('kode_user', $kode_user);
        if ($cek_user->count() > 0) {
            return redirect()->route('datauser')->with(['warning' => 'Gagal Menyimpan!! Username ' . $nama_user . ' Sudah Ada Telah Terdaftar']);
        } else {
...
//                    "email" => "default@gmail.com",
                    "password" => $password,
                    "roles" => $jenis_hakakses,
                    "foto" => $foto_user,
                    "created_at" => date('Y-m-d H:i:s'),
                ];
                $simpan = DB::table('salaman')->insert($data_tosave);
                if ($simpan) {
                    if ($request->hasFile('foto_user')) {
                        $folderPath = "public/uploads/salamanuser";
                        $request->file('foto_user')->storeAs($folderPath, $foto_user);
                    }
                    return redirect()->route('datauser')->with(['success' => 'Data Berhasil Disimpan']);
                } else {
                    return redirect()->route('datauser')->with(['warning' => 'Data Gagal Disimpan']);
                }

            } catch (Exception $e) {
                return redirect()->route('datauser')->with(['warning' => 'Data Gagal Disimpan Bor, error: ' . $e]);
    ...
    public function useredit(Request $request)
    {
        ...
        $user = DB::table('salaman')->where('kode_user', $kode_user)->where('id', $id_user)->first();
