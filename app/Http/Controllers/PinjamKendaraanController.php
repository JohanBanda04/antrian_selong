...
class PinjamKendaraanController extends Controller
{
    ...
    {
        $default_kodebooking = "";
        $pinjamKendaraans = DB::table('pinjam_kendaraan');
        if (!empty($request->kode_booking_cari)) {
            $default_kodebooking = $request->kode_booking_cari;
            $pinjamKendaraans->where('kode_booking', $request->kode_booking_cari);

        }

        $jenis_kendaraan_cari = $request->jenis_kendaraan_cari;
        $pinjamKendaraans->orderBy('created_at', 'desc');
        $dtpinjamKendaraans = $pinjamKendaraans->paginate(5);
        return view('pinjamkendaraan.index', compact('dtpinjamKendaraans'));

    }

    public function pinjamkendaraanedit(Request $request)
    {
        $kode_booking = $request->kode_booking;
        $pinjamkendaraan = DB::table('pinjam_kendaraan')->where('kode_booking', $kode_booking)->first();
        return view('pinjamkendaraan.edit', compact('pinjamkendaraan'));
    }

    public function pinjamkendaraanupdate(Request $request, $kode_booking)
    {
        //echo "woy"; die;
        //return $kode_booking;
        $keep_data = [];
        ...
        $pilih_persetujuan = $request->pilih_persetujuan;
        ...
        $kode_kendaraan_dipinjam = $request->kode_kendaraan_dipinjam;
        $get_plat = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan_dipinjam)->first()->plat_kendaraan;

        if ($pilih_persetujuan == "Ditolak") {
            $cek_pinjamkendaraan = DB::select(DB::raw("select * from pinjam_kendaraan 
where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_kendaraan='$kode_kendaraan_dipinjam' 
and kode_booking='$kode_booking_pinjam' and id='$id_kodebooking_pinjam'"));
            ...
            for ($i = 0; $i < $numberDays; $i++) {
                ...
                //echo $tgl_torent."<br>";
                //echo $kode_kendaraan_dipinjam."<br>";
                //echo $pilih_persetujuan."<br>";
                //echo date('Y-m-d')."<br>";
                //die;
                $delete = DB::table('pivot_pinjamkendaraan')
                    ->where('kode_kendaraan', $kode_kendaraan_dipinjam)
            ...
            try {
                ...
                $update = DB::table('pinjam_kendaraan')
                    ->where('kode_booking', $kode_booking_pinjam)
                    ->where('id', $id_kodebooking_pinjam)->update($data_tosave);

                if ($update) {
                    return redirect()->route('pinjamkendaraan')->with(['success' => 'Data Berhasil Disimpan']);
                } else {
                    return redirect()->route('pinjamkendaraan')->with(['warning' => 'Data Gagal Disimpan']);
                }
            } catch (Exception $e) {
                return redirect()->route('pinjamkendaraan')->with(['warning' => 'Data Gagal Disimpan, error : ' . $e]);
            }

        } else if ($pilih_persetujuan == "Disetujui") {
            //echo "masuk sini,disetujui woy"; die;
            $cek_pinjamkendaraan = DB::select(DB::raw("select * from pinjam_kendaraan 
where mulai_tgl='$mulai_tglpinjam_sql' and sampai_tgl='$sampai_tglpinjam_sql' and kode_booking='$kode_booking_pinjam'"));
            $get_oldstatus = $cek_pinjamkendaraan[0]->status;
            $get_datakendaraan = DB::table('kendaraan')->where('kode_kendaraan', $kode_kendaraan_dipinjam)->first();
            $get_platkendaraan = $get_datakendaraan->plat_kendaraan;
            $get_namekendaraan = $get_datakendaraan->name_kendaraan;
            $jarakWaktu = abs(strtotime($mulai_tglpinjam_sql) - strtotime($sampai_tglpinjam_sql));
            $numberDays = $jarakWaktu / 86400;
            $numberDays = intval($numberDays) + 1;
            for ($i = 0; $i < $numberDays; $i++) {
...
where tgl_pinjam='$tgl_torent' and kode_kendaraan='$kode_kendaraan_dipinjam' and kode_booking='$kode_booking_pinjam'"));
                if (count($cek_pivot_pinjamkendaraan) > 0) {
                    array_push($keep_data, (object)[
                        "status" => "Booked",
                        "tanggal" => $tgl_torent,
                        "kode_kendaraan" => $kode_kendaraan_dipinjam,
            ...
            //echo count($makesure_by_thisbookingcode);die;
            if (($get_oldstatus == "Pengajuan Pinjaman") && (count($makesure_by_thisbookingcode) > 0)) {
                //echo "prettt";die;
                /*dia memang sudah punya bookingan di tbl pivot kendaraan*/
                try {
                    $data_tosaved = [
                        "status" => $pilih_persetujuan,
                        "updated_at" => date('Y-m-d'),
                    ];
                    $update_pinjamkendaraan = DB::table('pinjam_kendaraan')
                        ->where('kode_booking', $kode_booking_pinjam)
                        ->where('id', $id_kodebooking_pinjam)->update($data_tosaved);
                    if ($update_pinjamkendaraan) {
                        return redirect()->route('pinjamkendaraan')->with(['success' => "Berhasil Mengupdate Data"]);
                    } else {
                        return redirect()->route('pinjamkendaraan')->with(['warning' => "Gagal Mengupdate Data"]);
                    }
                } catch (Exception $e) {
                    return redirect()->route('pinjamkendaraan')->with(['warning' => "Berhasil Mengupdate Data, error :" . $e]);
                }
                ...
                //echo "brey: " . count($makesure_by_thisbookingcode) . "<br>";
                for ($i = 0; $i < $numberDays; $i++) {
                    if ($i > 1) {
                        $tgl_torent = date('Y-m-d', strtotime($mulai_tglpinjam_sql . '+' . $i . ' days'));
                    } else if ($i <= 1) {
                    ...
                    $cek_pivot_pinjamkendaraan2 = DB::select(DB::raw("select * from pivot_pinjamkendaraan 
where tgl_pinjam='$tgl_torent' and kode_kendaraan='$kode_kendaraan_dipinjam'"));
                    if (count($cek_pivot_pinjamkendaraan2) > 0) {
                        array_push($keep_data2, (object)[
                            "status" => "Booked",
                            "tanggal" => $tgl_torent,
                            "kode_kendaraan" => $kode_kendaraan_dipinjam,
                        ]);
                    } else if (count($cek_pivot_pinjamkendaraan2) <= 0) {
                        //echo "bisa masuk ke tbl pivot";
                        array_push($keep_data2, (object)[
                            "status" => "Available",
                            "tanggal" => $tgl_torent,
                            "kode_kendaraan" => $kode_kendaraan_dipinjam,
                ...
                foreach ($keep_data2 as $index2 => $kd2) {
                    if ($kd2->status == "Booked") {
                        $get_plat = DB::table("kendaraan")
                            ->where("kode_kendaraan", $kd2->kode_kendaraan)->first();
                        array_push($filter_booked_date2, (object)[
                            "status" => "fix_booked",
                            "tgl_booked" => $kd2->tanggal,
                            "plat_kendaraan" => $get_plat->plat_kendaraan,
                            "jenis_kendaraan" => $get_plat->jenis_kendaraan,
                            "kode_kendaraan" => $kd2->kode_kendaraan,
                        ]);
                ...
                if (count($new_filtered2) > 0) {
                    ...
                    //echo "<pre>"; print_r($filter_booked_date2);
                    foreach ($filter_booked_date2 as $ind2 => $dta2) {
                        $tgl_booked = $dta2->tgl_booked;
                        $jenis_kendaraan = $dta2->jenis_kendaraan;
                        $tanggal_gagal2 .= $tgl_booked . ", ";
                        //echo $jenis_kendaraan."<br>";
                    }
                    //echo $tanggal_gagal2."<br>";
                    $warning = "Gagal Menyetujui!!! Kendaraan " . $get_namekendaraan . " dengan plat " . $get_platkendaraan . " telah dibooking user lain pada " . $tanggal_gagal2 . '. Mohon memilih range tanggal peminjaman yang lain';
                    return redirect()->route('pinjamkendaraan')->with(['warning' => $warning]);
                    ...
                    try {
                        ...
                        if ($update_pinjamkendaraan) {
                            foreach ($filter_available_date2 as $idx2 => $dtx2) {
                                $data_pivotkendaraan = [
                                    "kode_kendaraan" => $dtx2->kode_kendaraan,
                        ...
                        return redirect()->route('pinjamkendaraan')->with(['warning' => 'Data Gagal Disimpan, dgn error :' . $e]);
                    }
                }
            } else if(($get_oldstatus == "Disetujui") && (count($makesure_by_thisbookingcode) > 0)){
                //echo "nothing to do"; die;
                return redirect()->route('pinjamkendaraan')->with(['success'=>'No change has been made']);
            } else if(($get_oldstatus == "Disetujui") && (count($makesure_by_thisbookingcode) <= 0)){
                echo "save aja woy klo emg gada yg booking"; die;
            }
            //die;
...
where tgl_pinjam='$tgl_torent' and kode_kendaraan='$kode_kendaraan_dipinjam'"));
                ...
                if (count($new_filtered2) > 0) {
                    ...
                    //echo $tanggal_gagal2."<br>";
                    $warning = "Gagal Pengajuan Pinjaman!!! Kendaraan " . $get_namekendaraan . " dengan plat " . $get_platkendaraan . " telah dibooking user lain pada " . $tanggal_gagal2 . '. Mohon memilih range tanggal peminjaman yang lain';
                    return redirect()->route('pinjamkendaraan')->with(['warning' => $warning]);
                } else if (count($new_filtered2) <= 0) {
                    //echo "<br>ditolak to setuju berhasil";
                    try {
                ...
                //echo "nothing to do Pengajuan Pinjaman"; die;
                return redirect()->route('pinjamkendaraan')->with(['success'=>'No change has been made for Pengajuan Pinjaman']);
            } else if (($get_oldstatus == "Ditolak") && (count($makesure_by_thisbookingcode) > 0)) {
    ...
    public function pinjamkendaraandelete($kode_booking)
    {
        $pinjamkendaraan = DB::table('pinjam_kendaraan')->where('kode_booking', $kode_booking)->first();
        $mulai_tglpinjam_sql = $pinjamkendaraan->mulai_tgl;
        $sampai_tglpinjam_sql = $pinjamkendaraan->sampai_tgl;
        $kode_kendaraan_dipinjam = $pinjamkendaraan->kode_kendaraan;
        ...
        $daduk_to_delete = $pinjamkendaraan->datadukung;
        $folderPathOld = "public/uploads/datadukung/" . json_decode($daduk_to_delete);
        $delete = DB::table('pinjam_kendaraan')->where('kode_booking', $kode_booking)->delete();
        if ($delete) {
            for ($i = 0; $i < $numberDays; $i++) {
                if ($i > 1) {
                    $tgl_torent = date('Y-m-d', strtotime($mulai_tglpinjam_sql . '+' . $i . ' days'));
            ...
            return redirect()->route('pinjamkendaraan')->with(['success' => 'Data Berhasil Dihapus!']);
        } else {
            return redirect()->route('pinjamkendaraan')->with(['warning' => 'Data Gagal Dihapus!']);
        }
        //die;
        //return $kode_booking;
    }

    public function tampilkandetailpinjamkendaraan(Request $request)
    {
        $kode_booking = $request->kode_booking;
        $id_booking = $request->id_booking;
        $datapinjamkendaraan = DB::table('pinjam_kendaraan')
            ->where('kode_booking', $kode_booking)
            ->where('id', $id_booking)->first();

        //echo "<pre>"; print_r($datapinjamkendaraan); die;
        //dd($datapinjamkendaraan);
        return view('pinjamkendaraan.detail', compact('datapinjamkendaraan'));
    }
